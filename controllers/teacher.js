...
exports.findOne = function(req, res, next) {
	db.query("SELECT * FROM teacher WHERE userid=$1", [req.params.userid], function(err, rows) {
		if (err) return next(err);
		if (rows.length === 0) {
...
exports.insert = function(req, res, next) {
	...
	db.query("INSERT INTO teacher(userid,username,password,type,employeeno,name) VALUES($1, $2, $3, $4, $5, $6)", [req.body.userid, req.body.username, req.body.password,req.body.type,req.body.employeeno,req.body.name], function(err, row) {
		if (err) return next(err);
...
var selectOne = function(id, callback) {
	db.query("SELECT * FROM teacher WHERE userid=$1 LIMIT 1", [id], function(err, rows) {
		if (err) return next(err);
		if (rows.length === 0) {
			callback(null);
		} else {
